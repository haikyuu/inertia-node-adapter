...
export type FlashMiddleware = <FlashMessageType extends string>() => Middleware<
  ParameterizedContext<{ flash: Flash<FlashMessageType> }, DefaultContext>
>;
 ...
 */
const flash: FlashMiddleware = <FlashMessageType extends string>() => {
  return async (ctx, next: Next) => {
    ...
    class flash {
      ...
      setFlashMessage(type: FlashMessageType, message: string) {
        const messagesRecord: MessagesRecord = (ctx.session!.flashMessages =
